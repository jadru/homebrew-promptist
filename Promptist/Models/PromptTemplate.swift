...
struct PromptTemplate: Identifiable, Codable, Hashable {
    ...
    var title: String
    var content: String
    var keywords: [String]           // Search metadata (formerly 'tags')
    var linkedApps: [PromptAppTarget]
    var sortOrder: Int
    var usageCount: Int
    var lastUsedAt: Date?
    var collectionId: UUID?
    var categoryId: UUID?            // Category reference for hierarchical filtering
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case content
        case keywords
        case tags                    // Legacy key: maps to keywords
        case linkedApps
    ...
    init(from decoder: Decoder) throws {
        ...
        // Try 'keywords' first, fallback to legacy 'tags'
        if let decoded = try? container.decode([String].self, forKey: .keywords) {
            keywords = decoded
        } else {
            keywords = try container.decodeIfPresent([String].self, forKey: .tags) ?? []
        }
